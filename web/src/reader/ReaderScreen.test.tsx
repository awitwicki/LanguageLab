...
async function openReader(store = new MemoryBookStore()) {
  ...
  const onBack = vi.fn()
  const view = await render(<ReaderScreen hash={HASH} store={store} onBack={onBack} onOpenDictionary={vi.fn()} />)
  await flush()
  await flush()
...
describe('ReaderScreen', () => {
  it("opens where the server's position is, with the chapter and book in the header", async () => {
    ...
    // The bookmark sits at the saved sentence.
    expect(container.querySelector('[data-pos="1.2.0"] .reader-bookmark')).not.toBeNull()
  })
