...
- [x] Home screen's "Continue" reading row appears only when the last-read book's file is on this device (`web/src/reader/useContinueReading.ts`) — a book read on another device is silently left out, rather than offering a way into the library's "Open the file to continue" picker for it
- [x] Reader library: arriving from the home screen's "Open the file" row opens the file picker from an effect (`continueHash` in `web/src/reader/ReaderLibraryScreen.tsx`), which a browser may refuse once the tap that started it has gone stale — the book is then the first row under "On other devices" and nothing says why no dialog appeared; a notice naming it would explain the silence
...
- [ ] The alphabet screen: the ejective mark `ʼ` has no recording at all, and the diphthongs and the other marks have only their example word — Commons has no clip of a sound that is a modifier or a glide (`generate_ipa_catalog.py`, 108 of 124 symbols have a sound of their own)
