...
- The irregular-verbs trainer is a separate domain from dictionaries: the 68 verbs and their
  ...
  case-sensitively. The table's colour comes from `Mastery` as a band (`mastery.ts`,
  `--mastery-*` tokens), with the bar's length carrying the exact level. No `WordPair`, no
  dictionary, no seeding.
