...
- The pronunciation trainer is a separate domain from dictionaries and from the irregular-verbs
  ...
  `web/public/pronunciation-audio/`), grouped into sound families unlocked one at a time
  (`PronunciationLearningPath`). A user's standing on a word (`PronunciationProgress`:
