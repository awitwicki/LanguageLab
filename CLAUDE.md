...
- `LanguageLab.Application/` — services on top of the domain: word selection, training sessions, book import, sorting, per-scope Leitner progress (`LearningProgressService`), the irregular-verbs program (`IrregularVerbService`). Used by the API.
...
- The irregular-verbs program is a separate domain from dictionaries: the 68 verbs live in code (`LanguageLab.Domain/IrregularVerbs/IrregularVerbTable`), grouped into 4 pattern steps. Grades are stored per verb form (`IrregularVerbFormProgress` — streak, correct, wrong; a form is learned at `FormStates.LearnedStreak` = 3 in a row; `VerbProgress` folds a verb's three rows) and drive a weakest-first session of up to 10 verbs with the strongest form open (`IrregularVerbSessionPlanner`) via `/api/irregular-verbs` (`GET /`, `GET /steps/{n}/session`, `POST /grades`). No `WordPair`, no dictionary, no seeding — the seeded triplet dictionary was deleted by the `IrregularVerbProgram` migration.
